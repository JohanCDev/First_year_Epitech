...
void print_ld_info(char **array, t_asm *s_asm, char *hex_result)
{
...
void check_ld(char **array, t_asm *s_asm)
{
    int i = 0;
    char *hex_result = malloc(sizeof(char) * (9));

    for (i = 0; array[i]; i++);
    if (!my_strcmp(array[0], "ld")) {
        if (i != 3) {
            write(2, "ahhhh no, don't agree\n", 23);
            s_asm->ret = 84;
            return;
        ...
        print_ld_info(array, s_asm, hex_result);
    } else
        return;
}