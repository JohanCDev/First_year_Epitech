...
void print_binary_results(char **array, t_asm *s_asm, char *hex_result)
{
...
void check_and_or_xor(char **array, t_asm *s_asm)
{
    int i = 0;
    char *hex_result = malloc(sizeof(char) * (9));

    for (i = 0; array[i]; i++);
    if (!my_strcmp(array[0], "and") || !my_strcmp(array[0], "or") || \
    !my_strcmp(array[0], "xor")) {
        if (i != 4) {
            write(2, "ahhhh no, don't agree\n", 23);
            s_asm->ret = 84;
            return;
        ...
        print_binary_results(array, s_asm, hex_result);
    } else
        return;
}