...
employee_t *sort_node_emps_by_job_and_then(employee_t *before, \
...
employee_t *sort_node_emps_by_job(employee_t *before, \
employee_t *emp1, employee_t *emp2)
{
    if (my_strcmp(emp1->position, emp2->position) > 0) {
        swap_emp_nodes(before, emp1, emp2);
    } else if (my_strcmp(emp1->position, emp2->position) == 0 && \
    my_strcmp(emp1->last_name, emp2->last_name) > 0) {
        swap_emp_nodes(before, emp1, emp2);
    } else
        sort_node_emps_by_job_and_then(before, emp1, emp2);
    return emp1;
}