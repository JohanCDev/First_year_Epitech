...
employee_t *sort_node_emps_by_forename_and_then(employee_t *before, \
...
employee_t *sort_node_emps_by_forename(employee_t *before, \
employee_t *emp1, employee_t *emp2)
{
    if (my_strcmp(emp1->first_name, emp2->first_name) > 0) {
        swap_emp_nodes(before, emp1, emp2);
    ...
    && my_strcmp(emp1->last_name, emp2->last_name) > 0) {
        ...
        sort_node_emps_by_forename_and_then(before, emp1, emp2);
    return emp1;
}