...
{
    my_printf("This %d %i %s %X %x %b %S %o %u %p %P %% %#x %#X %#o", 5, 5, \
    "Hey", 12, 12, 12, "H\tn", 115, 2, 10, 10, 12, 12, 115);
    my_printf("%+i %+d %c %+i", 12, 12, 'a', -12);
...
Test(test_my_putnonprintable, my_putstr_nonprintable, .init = redirect_all_std)
{
...
{
    char **dest = \
    my_str_to_word_array("    test         random string  salt", " ");
